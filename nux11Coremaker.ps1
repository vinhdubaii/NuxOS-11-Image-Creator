...
Write-Host "Welcome to nuxos11 core builder! BETA 09-05-25"
Write-Host "This script generates a significantly reduced Windows 11 image. However, it's not suitable for regular use due to its lack of serviceability - you can't add languages, updates, or features post-creation. NuxOS11 Core is not a full Windows 11 substitute but a rapid testing or development tool, potentially useful for VM environments."
...
& dism /English "/mount-image" "/imagefile:$($env:SystemDrive)\nuxos11\sources\install.wim" "/index:$index" "/mountdir:$($env:SystemDrive)\scratchdir"
...
$imageInfo = & 'dism' '/English' '/Get-WimInfo' "/wimFile:$($env:SystemDrive)\nuxos11\sources\install.wim" "/index:$index"
...
if ($input -eq 'y') {
    ...
    & 'dism'  "/image:$scratchDir" '/enable-feature' '/featurename:NetFX3' '/All' "/source:$($env:SystemDrive)\nuxos11\sources\sxs" 
...
& 'dism' '/English' '/Export-Image' "/SourceImageFile:$mainOSDrive\nuxos11\sources\install.wim" "/SourceIndex:$index" "/DestinationImageFile:$mainOSDrive\nuxos11\sources\install2.wim" '/compress:max'
Remove-Item -Path "$mainOSDrive\nuxos11\sources\install.wim" -Force >null
Rename-Item -Path "$mainOSDrive\nuxos11\sources\install2.wim" -NewName "install.wim" >null
...
& 'dism' '/English' '/mount-image' "/imagefile:$mainOSDrive\nuxos11\sources\boot.wim" '/index:2' "/mountdir:$mainOSDrive\scratchdir"
Write-Host "Loading registry..."
...
& dism /Export-Image /SourceImageFile:"$mainOSDrive\nuxos11\sources\install.wim" /SourceIndex:1 /DestinationImageFile:"$mainOSDrive\nuxos11\sources\install.esd" /Compress:recovery
Remove-Item "$mainOSDrive\nuxos11\sources\install.wim" > $null 2>&1
